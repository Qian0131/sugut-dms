...
function renderFocQueue(){
  ...
  h+='<div class="sec" style="margin-top:12px">🎁 '+esc(tr('foc_give','Record fruit going out free'))+'</div>'+
     ...
     '<button class="big" onclick="focSubmit()">'+
       esc(may?tr('foc_record','RECORD IT'):tr('foc_ask','ASK THE GATE'))+'</button>';

  /* ---- 3. the book and the balance — MANAGEMENT ONLY.
       Both are farm-wide totals and both are built round what the fruit was WORTH, so with
       the money stripped out they would be a table of somebody else's rations. A worker's
       screen ends at his own requests and the form. ---- */
  if(!SHOW_VALUES){
    box.innerHTML=h;
    focQuote();
    return;}
