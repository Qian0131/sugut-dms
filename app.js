...
let EV_STALE_GS=false;   // v3.30.0 - the Sheet is running an Apps Script older than the return road
...
/* ======================= v3.30.0 JOB 1b · THE SILENT LINK WATCH ======================
   RESTORED from v3.19.3, which the v3.20.0 rebase silently reverted. Missing for ten
   releases, so between v3.20.0 and v3.29.0 the ONLY automatic upload trigger was the
   browser firing an 'online' event - and navigator.onLine reports true for a Wi-Fi
   association carrying no traffic, which is exactly what the yard looks like at the edge
   of range. A phone that filled its queue in the field and walked into the hotspot
   uploaded nothing unless somebody remembered to press the button.

   THE 30-SECOND TICK IS LOCAL AND FREE - it reads the QUEUE, not the network. A real
   handshake against the endpoint every 30s was once specified and would have broken the
   farm: every doGet is an Apps Script execution against a daily runtime budget measured
   in minutes, and four devices probing on that interval is ~11,500 executions a day. The
   quota would be gone by mid-morning and sync would stop for everyone - the exact failure
   the feature exists to prevent. So it touches the network only when there is something
   to send, and the call it then makes is the sync itself.

   AND IT GIVES UP HONESTLY. A queue that will not move must not be retried every 30
   seconds for ever; that is how a quota is burned by a bug rather than by work.
   Consecutive failures back off 30s - 1m - 2m - 4m - 10m - 20m, and after
   PROBE_PARK_AFTER fruitless rounds the watch parks itself until something real changes:
   the app is opened again, the link returns, or somebody presses SYNC NOW.
   ==================================================================================== */
...
/** Everything this device is still holding, by the same reckoning the badge uses. */
function probeQueue(){
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden)linkWake();});

async function linkWatch(){
  if(PROBE_PARKED)return;
  if(PROBE_SKIP>0){PROBE_SKIP--;return;}
  // Not logged in, killed, mid-sync, unnamed device or no endpoint: nothing to do, and no
  // network touched.
  if(LOCKED||syncing||!CFG||!CFG.url||!CFG.key)return;
  if(typeof deviceNamed==='function'&&!deviceNamed())return;
...
/* ===================== v3.30.0 JOB 2 · THE DEVICE NAME IS A LOCK =====================
   DEVICE_NAMES lives in database.js with the rest of the farm's fixed data. A device
   whose name is not on that list cannot UPLOAD - it can still record everything, so a
   wrong name costs an upload, never a day's work. The name is stored canonically (exactly
   as spelled in the list) so FIELD-A07 and field-a07 cannot become two devices on the
   Sheet, and so the v3.27.0 duplicate guard is comparing like with like.
   ==================================================================================== */
function deviceCanon(s){
  const t=String(s==null?'':s).trim();
  if(!t)return '';
  const list=(typeof DEVICE_NAMES!=='undefined'&&Array.isArray(DEVICE_NAMES))?DEVICE_NAMES:[];
  return list.find(d=>String(d).toLowerCase()===t.toLowerCase())||'';}
function deviceNamed(){return !!deviceCanon(CFG&&CFG.device);}
...
/* ===================== v3.30.0 JOB 3 - THE RETURN ROAD, DEVICE SIDE ====================
   THE BUG, in one line: refreshMasters() merged eleven master tables and never once wrote
   to EVENTS, so DROP / STOCK_IN / STOCK_OUT / STOCK_ADJUST / TIE / ROTTEN travelled UP
   only. Every balance in this app is a projection over the local event store, and a
   projection over a ledger that never receives the other device's rows cannot converge -
   it is not a race window, it never heals.

   THE RULES THIS MERGE OBEYS, and why each one matters:

   1. uuid is the identity. A row already here is NEVER overwritten - not its qty, not its
      cost, not its synced flag. The local copy may be mid-push, and a download must not
      be able to un-queue work that has not actually landed.
   2. syncedAt is left EMPTY on purpose. countsLocally() reads it to decide whether a local
      ...
      total it is already inside. The inventory side has no aggregate, so there the same
      row IS counted - which is exactly the point of this whole release.
   3. fromSheet marks where it came from, so a screen can say "this is another device's
      work" and so a future clean-up can tell downloaded rows from keyed ones.
   4. The pull is throttled. Every doGet is an Apps Script execution against a daily budget
      measured in minutes; four devices asking on every sync would spend it by mid-morning.
      Sixty seconds between event pulls is far tighter than anyone can key a delivery.
   ==================================================================================== */
let EV_LAST_PULL=0, EV_TRUNC=false, EV_LAST_N=0, EV_LAST_AT_S='';
...
async function refreshMasters(){
  ...
  try{
    ...
    // v3.30.0 - `since` is what turns the return road on. Omitted when the throttle says
    ...
    /* v3.30.0 - THE OTHER DEVICES' WORK. Placed AFTER the tree totals so the ledgers are
       rebuilt once, against the freshest aggregate, rather than twice. */
    if(wantEv){
      ...
      if(inEv){
        ...
        EV_LAST_N=nEv;
        EV_STALE_GS=false;          // the reply carried an events key: the deploy took
        if(nEv){
          got.events=nEv;
          /* Repaints are wrapped because a screen the current role never opens may have no
             elements to write to, and a render throwing here would abort the rest of the
             refresh - including the staff-registry update the kill switch depends on. The
             records are already merged and on disk by this point; only the paint is at risk. */
          try{
          ...
          toast('\u21ba '+nEv+' record'+(nEv>1?'s':'')+' received from the other devices');}
        if(EV_TRUNC)toast('\u26a0 Too many records to send at once - press SYNC again to get the rest',1);}
      else if(!(j&&j.ok&&('events' in j))){
        // the Sheet answered but has no idea what `events` is: the Apps Script is the old one
        EV_STALE_GS=true;}}
...
async function saveSetup(){
  /* v3.30.0 JOB 2 - the device name is no longer free text with a factory default. It must
     be one of the four names in DEVICE_NAMES, and it is stored in that exact spelling. */
  const canon=deviceCanon($('su-device').value);
  if(!canon){
    toast('Choose which device this is - '+((typeof DEVICE_NAMES!=='undefined'?DEVICE_NAMES:[]).join(' / ')),1);
    return;}
  CFG=Object.assign({},CFG,{device:canon,url:$('su-url').value.trim()});
 ...
 */
/* v3.30.0 - two states that used to be invisible get one amber line each. Nothing at all
   in the normal case: a health panel that always says something is furniture. */
function deviceHtml(){
  if(typeof deviceNamed==='function'&&deviceNamed())return '';
  return '<div class="stuckbox"><div class="sh">\u26a0 This device has no name yet</div>'+
    '<div>Nothing can be uploaded until you choose which device this is, in Setup. '+
    'Your work is still being recorded and will go up as soon as the name is set.</div></div>';}
function parkedHtml(){
  if(typeof PROBE_PARKED==='undefined'||!PROBE_PARKED)return '';
  return '<div class="stuckbox"><div class="sh">\u1F553 Automatic sync has paused</div>'.replace('\u1F553','\ud83d\udd53')+
    '<div>Six tries in a row moved nothing'+(PROBE_LAST_AT?(' (last tried '+esc(PROBE_LAST_AT)+')'):'')+
    ', so the app stopped retrying to protect the Google quota. '+
    'It will start again by itself when the link returns or you press SYNC NOW.</div></div>';}
/* v3.30.0 - the one line that tells you whether the Apps Script deploy actually took.
   Without it the return road can be dead for a week and every screen still looks normal,
   which is precisely how the original bug survived ten releases. */
function returnRoadHtml(){
  if(EV_STALE_GS)
    return '<div class="stuckbox"><div class="sh">\u2717 The Google Sheet is running the OLD script</div>'+
      '<div>Your work is going UP, but the other devices\u2019 work is not coming back DOWN. '+
      'Paste AppsScript_code.gs into the Sheet and choose Deploy \u2192 New version. '+
      'Until then every device shows only its own figures.</div></div>';
  if(!EV_LAST_PULL)return '';
  return '<div class="okbox2"><b>\u2713 Return road open.</b> Last check '+esc(EV_LAST_AT_S)+
    ' \u00b7 '+EV_LAST_N+' record'+(EV_LAST_N===1?'':'s')+' received from the other devices.</div>';}
...
function renderSync(){
  ...
  const sh=$('synchealth'); if(sh)sh.innerHTML=deviceHtml()+returnRoadHtml()+parkedHtml()+stuckHtml()+syncHealthHtml();
  const ap=$('appendnote');
...
let syncing=false;
/* v3.30.0 — how many events ride in one POST. RESTORED from v3.19.3, which the v3.20.0
   rebase silently reverted; it had been missing for ten releases. 40 is deliberately well
   under anything the Apps Script struggles with: at peak a tree visit writes a handful of
   rows, so 40 is roughly a morning's work per chunk and a whole day still fits in a few
   round trips. Raising it buys nothing and costs resume granularity; lowering it multiplies
   round trips against the script's daily execution budget. */
const SYNC_CHUNK=40;
/* The silent link watch (see linkWatch, near netPull) drives doSync() on its own. A sync
   nobody asked for must not put a message on a worker's screen while they are counting
   fruit. qtoast() is toast() everywhere except inside an automatic pass. */
let SYNC_QUIET=false;
function qtoast(m,err){ if(!SYNC_QUIET)toast(m,err); }
async function doSync(auto){
  if(syncing)return;
  // A person pressing the button is the clearest possible signal that it is worth trying
  // again, so it always un-parks the silent watch and clears its backoff.
  if(!auto&&typeof linkWake==='function')linkWake();
  if(!CFG||!CFG.url){if(!auto)toast('Set the Sync URL in Settings first',1);return;}
  /* v3.30.0 JOB 2 — THE NAME TAG HAS TO BE REAL. The v3.27.0 duplicate guard compares
     e.device against the device that filed the job first; two machines both left on the
     factory default 'phone-01' read as ONE machine, so the guard waves the duplicate
     through. That is the 85 kg MSolumax case with the safety catch disengaged. Nothing is
     blocked from being RECORDED here — only from being SENT — so a wrong name costs an
     upload, never a day's work. */
  if(!deviceNamed()){
    if(!auto){toast('This device has no proper name yet — set it in Setup before syncing',1);
  ...
  if(!batch.length){
    ...
    return;}
  /* v3.30.0 JOB 1 - CHUNKED AND BANKED. Restored from v3.19.3 and merged with the v3.27.0
     duplicate-blocked report, which did not exist when the chunk loop was first written.
     WAS: one POST carrying the entire queue, with NO timeout - alone among the sync paths.
       - no resume    a dropped packet at event 390 of 400 left all 400 queued and the next
                      attempt started again from zero
       - no deadline  a half-open hotspot (Wi-Fi associated, carrying no traffic - the yard
                      at the edge of range) hung the button on "Uploading..." and, with no
                      finally, left `syncing` true so every later press did nothing until
                      the app was closed and reopened
     NOW: SYNC_CHUNK events per POST, marked synced the moment that chunk is acknowledged,
     so an interrupted upload RESUMES. Re-sending a chunk is harmless: the Apps Script
     dedupes on the client uuid through the hidden SYNC_INDEX sheet, so a chunk that landed
     but whose reply was lost is absorbed, not duplicated. Every chunk is the same
     {events:[...]} body it always was - no Apps Script change for this part. */
  syncing=true;const b=$('syncbtn');
  let sent=0, chunkErr=null, blockedN=0, blockedRows=[];
  try{
    for(let i=0;i<batch.length;i+=SYNC_CHUNK){
      const part=batch.slice(i,i+SYNC_CHUNK);
      if(b)b.textContent='Uploading '+(sent+part.length)+'/'+batch.length+'…';
      ...
      for(let attempt=1;attempt<=SYNC_TRIES;attempt++){
        try{
          ...
          if(j&&j.ok){
            landed=true;
            // v3.27.0 rows the server refused because another device had already filed that
            // exact job. Gathered across every chunk and reported ONCE, at the end.
            if(j.blocked>0){blockedN+=j.blocked;
              (j.blockedRows||[]).forEach(x=>blockedRows.push(x));}
            break;}
    ...
    renderSync();
    /* The duplicate report must NOT be a toast that fades in two seconds: somebody has just
       done work the farm already has, and they need to stop and check rather than key it
       again. Shown even when a later chunk failed - those rows are still blocked. */
    if(blockedN>0){
      const rows=blockedRows.map(x=>'• '+(x.product||'?')+' — '+(x.qty||'')+' '+(x.unit||'')+
        (x.lot?(' · Lot '+x.lot):'')+(x.set?('\n   '+x.set):'')).join('\n');
      alert('⚠ '+blockedN+' row'+(blockedN>1?'s were':' was')+' NOT saved — already filed on another device.\n\n'+
        ...
        'time, file it from the SAME device that filed it first.');}
    if(chunkErr){
      noteSyncFail('events','Field records',batch.length-sent,chunkErr);
      badge(); renderSync();
      ...
      qtoast(left?('⚠ '+sent+' sent, but '+left+' '+tr(left>1?'sy_stuck_n':'sy_stuck_1'))
                 :(blockedN>0?('✓ '+(sent-blockedN)+' synced · '+blockedN+' duplicate blocked')
                             :('✓ '+sent+' events synced to Google Sheets')),!!left||blockedN>0);
      // v3.30.0 Job 3 - and now bring the OTHER devices' work back down.
      await refreshMasters();
    }
  }catch(err){qtoast('Sync failed: '+err.message,1);renderSync();}
  finally{ syncing=false; if(b)b.textContent='⇧ SYNC NOW'; }}
...
function tiedLoggedOf(tree){
  let n=0;
  EVENTS.filter(e=>e.type==='TASK_DONE'&&e.kind==='FTIE'&&countsLocally(e)).forEach(e=>{
