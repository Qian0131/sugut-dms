...
const MODULES={
  ...
  cmd:{ic:'👑',name:'Command',sub:'today, summary, compare, programme, master',tn:'m_cmd',
    tabs:[{k:'home',  t:'TODAY',           scr:'dash',panels:['ownerhome'], roles:['OWNER'],ic:'📌',tn:'s_today', d:'Today, the week or the whole season — drop, quality and what is left on the trees'},
          {k:'today', t:'OLD TODAY CARD',  scr:'dash',panels:['cmdtoday'],  roles:['OWNER'],ic:'🗂️', d:'The previous today card, kept until the new one has been through a season'},
 ...
 *  forecast — it is the number of fruit counted hanging on 5 July — but it was only taken on
 *  some of the trees, and a forecast built on half a farm must say so out loud rather than
 *  quietly read low. Trees with no census carry `census==null`, NOT zero. */
...
function ownSeasonChart(){
  ...
  return '<svg viewBox="0 0 '+W+' '+H+'" class="ownchart" role="img" aria-label="'+
    ...
    '<div class="small mtxnote">'+
      esc(tr('ow_projnote2','The amber line is your own July census — what the crew counted hanging. The grey dashes are not a plan: they are today’s rate carried forward, stopping at the fruit still on the trees.'))+
      (cen.full?'':(' <b>'+esc(tr('ow_censuspart','The census covered'))+' '+cen.counted+' '+
        esc(tr('ow_censusof','of'))+' '+cen.total+' '+esc(tr('ow_censustrees','trees'))+
        ' ('+cen.pct+'%), '+esc(tr('ow_censusbigger','so the real crop is larger than that line.'))+
        '</b>'))+
    '</div>';}
...
function ownTools(){
  const keep=hubTiles().filter(k=>k!=='cmd');
