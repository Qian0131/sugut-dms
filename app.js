...
function focReasonOpts(sel){
  return FOC_REASON_ORDER.map(r=>'<option value="'+r+'"'+(r===sel?' selected':'')+'>'+
    esc((FOC_REASONS[r].ic||'')+' '+FOC_REASONS[r].label)+'</option>').join('');}
...
function renderFocQueue(){
  ...
  const may=focMayDecide(), ym=now().slice(0,7), pend=focPending();
  let h='';
    ...
    h+=pend.map(e=>{
      ...
      return '<div class="focreq'+(over?' over':'')+'">'+
        '<div class="fr-top">'+
          ...
          '<div class="fr-who"><b>'+esc(meta.label||e.reason)+'</b>'+
          ...
          '<div class="fr-kg">'+nf(e.kg)+'<small>kg</small>'+
            '<small class="fr-rm">'+rm(e.value_rm)+'</small></div>'+
        '</div>'+
        // the value it would have fetched, said out loud before she taps
        '<div class="fr-note">'+esc(tr('foc_worth','Worth'))+' <b>'+rm(e.value_rm)+'</b> '+
          esc(tr('foc_atrate','at'))+' '+rm(e.rate_rm_kg)+'/kg'+
          (e.note?(' · '+esc(e.note)):'')+'</div>'+
        (cap.cap>0
          ? ('<div class="fr-note'+(over?' bad':'')+'">'+esc(meta.label)+' '+
  ...
  /* ---- 3. the book, by reason, against the allowance ---- */
  ...
  h+='<div class="sec" style="margin-top:14px">📗 '+esc(tr('foc_book','The book — this month'))+'</div>'+
     ...
     FOC_REASON_ORDER.map(r=>{
       ...
       return '<tr'+(meta.isLoss?' class="isloss"':'')+'><td>'+esc((meta.ic||'')+' '+meta.label)+'</td>'+
