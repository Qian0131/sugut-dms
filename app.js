...
const SETTINGS_KEYS=['cloneprice','pricemeta','baskets','tareok','addtrees',
  ...
  /* v3.28.0 — 'invover' WAS NEVER SHARED, and that was a real hole, not a new feature.
     INV_OVERRIDE carries the Owner's corrections to a product: its minimum stock level, its
     active ingredient, and now its price and its retired date. It was written to this phone's
     IndexedDB and NOWHERE ELSE, so Sandakan could confirm what is actually in the Ardel bottle
     and the Owner's phone would never learn it. Same family as the v3.11 shared-settings fix:
     a setting is not an event. It merges PER PRODUCT by `at`, like agrodrafts and progover —
     whole-object newest-wins would let one phone's edit erase another's. */
  'invover',
  // v3.21 — the Owner's corrections to a programme set. A collection keyed by phase id,
  // so it MERGES per record like agrodrafts, never whole-object newest-wins.
  'progover'];
...
/* v3.21 — PROG_OVER holds the Owner's changes to workbook sets, keyed by phase id:
   {at, by, removed:true} or {at, by, plan:'YYYY-MM-DD', lines:[{pid,qty}]}.
   PROG_SEED is the untouched programme as it left database.js, captured once. Every
   apply rebuilds PHASE_PROGRAM from the seed, so applying twice cannot compound and a
   change can always be undone. */
let PROG_OVER={}, PROG_SEED=null;
...
async function initStore(){
  ...
  if(db){ EVENTS=(await all('events'))||[]; const kv=(await all('kv'))||[];
    ...
    if(bk&&Array.isArray(bk.v)&&bk.v.length){
      // v3.29.7 - overlay the seed, then KEEP every basket the farm added itself.
      // Without the second half a restart wiped an added basket exactly like a sync did.
      const bseen={};
      const bmerged=BASKET_SEED.map(seed=>{
        bseen[String(seed.id)]=1;
        const saved=bk.v.find(x=>x&&String(x.id)===String(seed.id));
        return saved?{...seed,tare_kg:+saved.tare_kg||0,name:saved.name||seed.name}:{...seed};});
      bk.v.forEach(x=>{ if(!x||!x.id)return; const id=String(x.id); if(bseen[id])return;
        bseen[id]=1; bmerged.push({id:id,name:String(x.name||id),tare_kg:+x.tare_kg||0,ic:x.ic||'\ud83e\uddfa'});});
      BASKETS=bmerged;}
    ...
    // ---- v3.19.2 — the one-time 2026 usage-history import ----------------------------
    // The farm kept eight months of spray and fertiliser rounds in its own workbook and
    // the app held none of them, so every costing screen read RM 0 of input spend for the
    // whole season. These are ordinary STOCK_OUT events — no new type, no Apps Script
    // change — with FIXED uuids, so the six phones that all carry this file cannot make
    // six copies of the same round and the Sheet's SYNC_INDEX drops any that arrive
    // twice. They go in UNSYNCED on purpose, so the first phone to sync carries the year
    // up to the Sheet. Every one is stamped IMPORT 2026 / sheet-import and can always be
    // told apart from work the crew filed themselves. Runs exactly once (kv `usageimp`).
    const uim=kv.find(x=>x.k==='usageimp');
    if(typeof USAGE_IMPORT_2026!=='undefined'&&String(uim&&uim.v||'')!==USAGE_IMPORT_TAG){
      const have={}; EVENTS.forEach(e=>{have[e.uuid]=1;});
      let added=0;
      for(const row of USAGE_IMPORT_2026){
        if(have[row.uuid])continue;
        const e={...row}; EVENTS.push(e); if(db)await put('events',e); added++;}
      await put('kv',{k:'usageimp',v:USAGE_IMPORT_TAG});
      if(added)console.log('[DMS] 2026 usage history imported — '+added+' entries');}
  ...
  applyProgOverrides();                         // and the Owner's programme corrections
  rebuildLedgers();                             // materialise stock_in / stock_out / adjust ledgers
  /* v3.29.8 - honour clean-ups at boot as well. A device that was switched off when the
     removal synced would otherwise carry the dead rows until the next successful pull. */
  if(typeof applyTombstones==='function'){try{await applyTombstones();}catch(e){}}
}
...
/* v3.27.0 - THE NAME TAG. `uuid()` above is random, so when two phones file the SAME
   real-world job they mint two different ids and the Sheet's SYNC_INDEX dedupe - which
   keys on uuid alone - waves both through. That is exactly how directive DIR-001 set 2
   (MSolumax 3-16-36) was booked out TWICE on 4 and 5 Aug 2026, 85,500 gm and RM 908.44
   each, off ONE broadcast, leaving the store at -23,000 gm.

   A natural key is derived from WHAT THE ROW IS, not from when it was made, so both
   phones compute the same string for the same job and the second copy can be recognised.
   Deliberately NOT in the key: the timestamp, the device, the worker and the crew/hours -
   those differ between two filings of one job and would defeat the whole point.
   Deliberately IN the key: the tree count, because a genuine partial re-run of the same
   directive over a DIFFERENT number of trees is real work, not a duplicate, and must
   still be allowed through. */
function natKeyRun(progId,slot,lot,qty,trees){
  return 'RUN|'+progId+'|'+slot+'|'+lot+'|'+(+qty||0).toFixed(2)+'|'+(+trees||0);}
...
/* v3.29.4 — EVERY date in this app is written "YYYY-MM-DD HH:mm". One is not. Google Sheets
   parses the dt column of MKT_DISPATCH and AUDIT_LOG into a real Date cell, and the Apps
   Script serves it as String(dateCell) — "Wed Aug 05 2026 03:40:00 GMT+0800 (Singapore
   Standard Time)". Compared as text, "Wed Aug 05" sorts ABOVE "2026-08-07", because W beats
   2. So every decision that arrived from another phone passed any date test put to it and
   sorted to the top of any list, however old it was. That is why four 4-August trial loads
   were still sitting above a real one on the Gate phone after v3.29.3 supposedly windowed
   them out — the window was working, the dates were lying to it.
   normDt takes either form and returns ours. It is applied on the way IN so new rows store
   correctly, and on the way OUT because the rows already sitting in REQ_DECIDED on three
   phones are never rewritten. The origin is the Apps Script; fixing it there too would be
   tidier but needs a redeploy, and this heals the phones that already hold the bad text. */
function normDt(v){
  const s=String(v==null?'':v).trim(); if(!s)return '';
  if(/^\d{4}-\d{2}-\d{2}/.test(s))return s.slice(0,16);
  // Anything this short is not a date. new Date('0') is the year 2000 and new Date('5') is
  // May 2001 - a stray number in the column would otherwise become a confident wrong date.
...
const MODULES={
  ...
  cmd:{ic:'👑',name:'Command',sub:'today, summary, compare',tn:'m_cmd',
    tabs:[{k:'home',  t:'TODAY',           scr:'dash',panels:['ownerhome'], roles:['OWNER'],ic:'📌',tn:'s_today', d:'Today, the week or the whole season — drop, quality and what is left on the trees'},
          ...
          /* the card the landing replaced, kept LAST and named for what it is. It is the
             way back if the new page turns out to be missing something during the trial;
             it comes out for good once a season has run through the new one. */
          {k:'today', t:'OLD TODAY CARD',  scr:'dash',panels:['cmdtoday'],  roles:['OWNER'],ic:'🗂️', d:'The card the new TODAY page replaced — kept as a fallback for the trial'},
  ...
  mkt:{ic:'📊',name:'Gate & Merchants',sub:'approve, send out, price, read back',tn:'m_mkt',
    tabs:[{k:'verify',t:'LIVE DISPATCH REVIEW',scr:'dash',panels:['verifycard'], roles:FULL_ROLES,ic:'📷',tn:'s_review',d:'Check the scale photo against the keyed figures, then approve and dispatch'},
          {k:'foc',   t:'RATIONS & GIFTS',    scr:'dash',panels:['foccard'],   roles:FULL_ROLES,ic:'🎁',tn:'s_foc',d:'Fruit that leaves without an invoice — approve it, and see where every kilo went'},
...
const HUB_ORDER={
  ...
  MARKETING:['mkt','harvest','reports'],          // v3.24 — was the Owner's eight
...
const HOME_TILES={
  OWNER:['cmd','agro','reports','admin']   // TODAY · FARM · MONEY · ADMIN — was nine
};
...
async function applyTombstones(){
  ...
  if(!goneEv.length&&!goneCo.length)return 0;
  ...
  return goneEv.length+goneCo.length;}
...
async function mergeEvents(rows){
  ...
  /* v3.29.8 - build the tombstone list from what is here AND from what is arriving, so a
     ...
     not the one after. */
  const dead=deadSet();
  rows.forEach(r=>{ if(r&&r.dead)String(r.dead).split(/\s+/).forEach(u=>{if(u)dead[u]=1;}); });
  let n=0;
  for(const r of rows){
...
async function refreshMasters(){
  ...
  try{
    ...
    const q='?role='+encodeURIComponent(myRole())+
            ...
            (wantEv?('&since='+encodeURIComponent(SYNC_EVENTS_FROM)):'');
...
/* ================= v3.28.0 · THE WEATHER ENGINE LEARNS FORMULATION ==================
   rainClass() above keys on the ACTIVE INGREDIENT string alone. Agus 24SC and
   Pengasus 47.17sc both read "Diafenthiuron", so the app gave them IDENTICAL rain advice —
   even though holding both is precisely a rain-day / dry-day decision. The Owner's words,
   8 Aug 2026: "choosing different brand on the same AI is important due to either rain day
   or dry day... I have learn to add more other products and same AI due to weather and also
   cost." The formulation is in the product NAME (24SC, 1.8EC, 18SL, 80WB), so read it there.
   FORM_RANK is rainfastness, not quality: a suspension concentrate binds faster than an
   emulsifiable concentrate, and a soluble liquid is the first to wash off. */
const FORM_CODES=[
  [/\b\d*\.?\d*\s*(WG|WDG)\b/i,'WG','water-dispersible granule',3,'binds hard once dry'],
  [/\b\d*\.?\d*\s*(SC)\b/i,     'SC','suspension concentrate',   3,'binds well once dry'],
  [/\b\d*\.?\d*\s*(WP|WB)\b/i,  'WP','wettable powder',          2,'moderate once dry'],
  [/\b\d*\.?\d*\s*(EC)\b/i,     'EC','emulsifiable concentrate', 2,'moderate, needs drying time'],
  [/\b\d*\.?\d*\s*(SL)\b/i,     'SL','soluble liquid',           1,'washes off easily'],
  [/\b\d*\.?\d*\s*(EW|SE)\b/i,  'EW','emulsion in water',        2,'moderate once dry']];
/** The formulation code carried in a product name, or null when it does not declare one. */
function formOf(p){
  const n=String((p&&p.name)||'');
  for(const [re,code,label,rank,why] of FORM_CODES)
    if(re.test(n))return {code:code,label:label,rank:rank,why:why};
  return null;}
/** The strength a product declares in its own name — 24SC is 24%, 47.17sc is 47.17%. */
function strengthOf(p){
  const m=String((p&&p.name)||'').match(/(\d+(?:\.\d+)?)\s*(?:SC|EC|WG|WDG|WP|WB|SL|EW|SE)\b/i);
  return m?+m[1]:null;}
/** Rain behaviour for a PRODUCT: the ingredient's class, sharpened by its formulation. */
function rainClassOf(p){
  const base=rainClass((p&&p.active_ingredient)||'');
  const f=formOf(p);
  if(!f)return base;
  return {k:base.k, form:f.code, formLabel:f.label, formRank:f.rank,
          why:base.why+' \u00b7 '+f.code+', '+f.why};}
...
function systemicAlternatives(cat){
  return activeProducts().filter(p=>{   // v3.28.0
...
function bpPool(){const cats=BP_CATS[$('bp-kind').value]||[];
  // v3.28.0 — activeProducts(): a retired product must never be prescribable.
  return activeProducts().filter(p=>cats.indexOf(p.cat)>=0);}
function bpAIs(){
  const seen={},out=[];
...
function setStatus(p){
  ...
  // v3.20 — the farm's own programme sheet carries an ACTUAL date beside every PLAN, and
  // that is the only evidence for work finished before any of this existed. No directive
  // was ever issued for those sets, so nothing here could ever have called them done, and
  // the record showed a season of completed work as LATE. `fromFile` tells every caller
  // the evidence is the sheet, not a filing on a phone — so nothing pretends otherwise.
  if(!recs.length&&p.done)return {st:'DONE',on:p.done,rec:null,fromFile:true};
...
/** v3.20 — what the store spent against ONE programme set. Every imported 2026 entry
 *  carries the phaseId of the set it belongs to, so this is summed from the ledger and
 *  can never drift from it. Work filed on a phone against an issued directive keeps its
 *  own progId path and is not counted twice here. */
function phaseSpend(id){
  let rm=0,n=0; const lots={};
  for(const e of EVENTS){
    if(e.type!=='STOCK_OUT'||e.phaseId!==id)continue;
    rm+=(+e.cost||0); n++; if(e.lot)lots[e.lot]=1;}
  return {rm:+rm.toFixed(2),n:n,lots:Object.keys(lots).sort()};}

/** The record of a set finished before the app existed: when it started, when it
 *  finished, whether that beat the plan, and what left the store against it. Money is
 *  gated on SHOW_VALUES like every other RM figure. */
function phaseRecordHTML(p){
  const sp=phaseSpend(p.id);
  let h='<div class="varbox ok"><b>'+esc(tr('pr_fromsheet'))+'</b><br>';
  if(p.donePerLot){
    h+=LOT_KEYS.filter(L=>p.donePerLot[L])
        .map(L=>'Lot '+L+' '+esc(dateShort(p.donePerLot[L]))).join(' · ');
  } else if(p.started){
    h+=esc(tr('pr_started'))+' '+esc(dateShort(p.started))+' → '+
       esc(tr('pr_finished'))+' '+esc(dateShort(p.done));
  } else {
    h+=esc(tr('pr_finished'))+' '+esc(dateShort(p.done));
  }
  h+='<div class="exphint">'+(sp.n
      ?(sp.n+' '+esc(tr('pr_rows'))+(SHOW_VALUES?(' · '+rm(sp.rm)):'')+
        (sp.lots.length?(' · Lot '+sp.lots.join(', ')):''))
      :esc(tr('pr_nomaterial')))+'</div>';
  // The plan date now IS the finishing date, on the Owner's instruction: a round that
  // takes three days on a hand pump was never a late round, it was a full-length job.
  // The date first written in the workbook is kept and shown, so nothing is lost.
  if(p.planOriginal)
    h+='<div class="exphint">'+esc(tr('pr_replan'))+' '+esc(dateShort(p.planOriginal))+
       ' → '+esc(dateShort(p.plan))+'</div>';
  // where the workbook tick and the day material actually left the store disagree, the
  // store wins and the workbook date is printed beside it rather than quietly dropped
  if(p.sheetDone&&p.sheetDone!==p.done)
    h+='<div class="exphint">'+esc(tr('pr_sheetsaid'))+' '+esc(dateShort(p.sheetDone))+'</div>';
  if(p.unconfirmed&&p.unconfirmed.length)
    h+='<div class="exphint" style="color:#7a5b00">'+esc(tr('pr_unconf'))+': '+
       p.unconfirmed.map(esc).join(' · ')+'</div>';
  return h+'</div>';}
...
/** v3.21 — sets the Owner removed from THIS month, so there is always a way back. */
function tlRemoved(){
  return (PROG_SEED||[]).filter(p=>p.month===tlMonth&&PROG_OVER[p.id]&&PROG_OVER[p.id].removed);}
function tlSets(){
...
function tlCard(p){
  ...
  let inner='';
  if(isOpen){
    // v3.20 — a set known finished from the programme sheet is history. Asking the Owner
    // to go and ACTIVATE it so a tank count can be worked out is noise on a job that
    // ended five months ago, so the prompt is suppressed there and only there.
    inner=((s.fromFile&&!(lpt>0))?'':tankLineHTML(p,lpt,trees))+comboGridHTML(p,lpt,trees);
    ...
    if(s.fromFile){ inner+=phaseRecordHTML(p); }
    else if((s.st==='DONE'||s.st==='PART')&&rec){
      const done=lotsDone(rec.uuid);
      inner+='<div class="varbox ok">Reported for '+done.map(L=>'Lot '+L).join(', ')+
        (s.on?(' · last on '+esc(s.on)):'')+'</div>';}
    // v3.21 — the Owner's own corrections. Gated on SHOW_VALUES, which is the Owner /
    // Marketing gate everywhere else in the app, so no worker or purchaser sees these.
    if(tlEdit===p.id){ inner+=progEditHTML(p); }
    else if(SHOW_VALUES&&myRole()==='OWNER'){
      inner+='<div class="cacts"><button class="no" onclick="event.stopPropagation();progOpenEdit(\''+esc(p.id)+'\')">'+
        esc(tr('pe_edit'))+'</button><button class="no pedel" onclick="event.stopPropagation();progRemove(\''+esc(p.id)+'\')">'+
        esc(tr('pe_remove'))+'</button></div>';}
    if(tlEdit!==p.id)inner+='<div class="cacts">'+
      (s.st==='DONE'
...
function renderTimeline(){
  ...
  sets.forEach(p=>{
    ...
    rows+='<div class="tlrow"><div class="tldate">'+(p.plan?esc(p.plan.slice(8)):'—')+
      // v3.20 — this used to slice the ROUND NAME, so the Boosting round printed "Boo".
      // A round is not a calendar month; take the month from the plan date itself.
      '<small>'+(p.plan?esc((LANG==='ms'?MONTH_LONG_MS:MONTH_LONG_EN)[+p.plan.slice(5,7)-1].slice(0,3)):'no date')+'</small></div>'+
      '<div class="tldot '+st.toLowerCase()+'"></div>'+tlCard(p)+'</div>';});
  // removed sets sit under the month, greyed, with one button to put them back
  if(SHOW_VALUES&&myRole()==='OWNER')tlRemoved().forEach(p=>{
    rows+='<div class="tlrow"><div class="tldate">'+(p.plan?esc(p.plan.slice(8)):'—')+
      '<small>'+(p.plan?esc((LANG==='ms'?MONTH_LONG_MS:MONTH_LONG_EN)[+p.plan.slice(5,7)-1].slice(0,3)):'')+'</small></div>'+
      '<div class="tldot"></div><div class="cbox perem"><div class="cbt">'+esc(p.set)+'</div>'+
      '<div class="cbs">'+esc(tr('pe_removedlbl'))+'</div>'+
      '<div class="cacts"><button class="no" onclick="progRestore(\''+esc(p.id)+'\')">'+
      esc(tr('pe_restore'))+'</button></div></div></div>';});
...
function renderTying(){
  ...
  box.innerHTML=
    ...
    '<p class="small">Every fruit here comes from a dated <b>Fruit tying</b> record — the field book backdated on '+
    '9 Aug 2026, plus every reply the workers have sent since. The July census opening balance was retired that day, '+
    'because the book already covered it. Each drop and each rotten fruit comes off automatically — nobody keeps this figure by hand.</p>';}
...
async function retryOne(key){
  const m={rotten:pushRotten,logadj:pushLogAdj,tying:pushTying,tieadj:pushTieAdj,
    ...
    // v3.30.0 — without this line the RETRY button on a stuck "Rations & gifts" row would
    // say "Nothing to retry". Same dead-end the settings key had in v3.11.
    foc:pushFoc};
...
let savingTie=false;
/* v3.26.1 — with ROPE_TRACKING off these both return 0, and that single fact switches the whole
   feature off cleanly: commitTieRound()'s `if(rp&&need>0)` stops writing the rope STOCK_OUT, the
   `have<need` shortage confirm can never fire, and the tying tile's `rope<0` red badge goes
   quiet. The tying count itself is untouched — only the rope that used to ride along with it. */
function ropeNeeded(n){if(!ROPE_TRACKING)return 0;return +((+n||0)*ROPE_M_PER_FRUIT).toFixed(2);}
function ropeOnHand(){if(!ROPE_TRACKING)return 0;const p=prodById(ROPE_PID);return p?onHand(p):0;}
/**
...
function tyPaint(){
  ...
  const rope=$('ty-rope');
  if(rope&&!ROPE_TRACKING){rope.innerHTML='';}      // v3.26.1 — not tracked this season
  else if(rope){
...
async function tySave(){
  ...
  toast('🎗️ '+n+' tied @ '+t.id+(ROPE_TRACKING?(' · '+nf(ropeNeeded(n))+' m rope drawn'):'')+
...
function ropeCardHTML(){
  if(!ROPE_TRACKING)return '';            // v3.26.1 — not tracked this season
...
/* v3.26.0 FIX (audit D-16) — this was the ONE aggregator in the ledger missing the
   countsLocally() gate. Every sibling has it — securedDropsOf, untiedDropsOf, rottenTiedOf,
   rottenUntiedOf, tieRoundsOf — because a row that has already been counted into the farm-wide
   TREE_STATS snapshot must not then be added again from this phone's own log. tiedLoggedOf
   adds tieRoundsOf(), which ALREADY includes statOf(tree,'tied') from that snapshot, and then
   piled ungated TASK_DONE rows on top. A worker replying "20 fruit tied on B-045" and syncing
   made the tree read 40: the drop forecast inflated, and the over-collection guard never fired
   because the phantom balance was always big enough to cover whatever was taken off. */
function tiedLoggedOf(tree){
  let n=0;
  /* v3.29.5 - `e.fromSheet||` is load-bearing, and the reason is subtle. countsLocally()
     exists to stop a local row being added on top of the Sheet's TREE_STATS aggregate,
     and a downloaded row carries an empty syncedAt precisely so it reads as "already in
     the aggregate". But readTreeStats_ reads TYING_LOGS, TIE_ADJUST, DROP_LOGS,
     ROTTEN_LOGS and LOG_ADJUST - it does NOT read TASK_LOGS. So fruit tied through an
     Owner-assigned TASK is in NO aggregate, and applying countsLocally to a downloaded
     one would silently drop it from the tied count. Every other type on the return road
     IS in the aggregate; this is the single exception. */
  EVENTS.filter(e=>e.type==='TASK_DONE'&&e.kind==='FTIE'&&(e.fromSheet||countsLocally(e))).forEach(e=>{
...
/* v3.29.7 - THE LABOUR RATE STAYS WITH THE OWNER. It is the one figure here that prices
   the farm's own people, and it belongs to whoever signs for them. */
function canSetPrice(){return myRole()==='OWNER';}          // labour rate

/* v3.29.7 - CREDIT AND THE RETAILER MASTER OPEN TO THE GATE. Asked for on 9 Aug 2026:
   the Gate is the device standing in front of the merchant when the lorry is loading, and
   an Owner out in the lot cannot post a top-up in time to release a load. Same two roles
   that already set prices and the basket tare - this does not widen who sees money, only
   who may move it, and every top-up is still an EVENT with its own signed audit row
   naming the device and the person. The labour rate is deliberately NOT included. */
function canSetCredit(){return FULL_ROLES.indexOf(myRole())>=0;}   // Owner + Marketing/Gate
 ...
 *  v3.30.0 — a SHAPE grade (banana) is never suggested from a weight, and can never
 *  be the fall-through answer either: it has no band, and the fall-through now picks
 *  the last WEIGHED letter rather than the last letter on the clone's ladder. Without
 *  that second guard, appending BN to CLONE_GRADES would have quietly made every
 *  unbanded weight come back as 'banana'. */
function gradeForWeight(clone,kg){
  kg=+kg||0; const gs=gradesFor(clone).filter(g=>bandOf(clone,g));
  for(let i=0;i<gs.length;i++){const b=bandOf(clone,gs[i]);
    if(kg>=b.min&&(b.max==null||kg<b.max))return gs[i];}
  return gs[gs.length-1]||'';}
/** Is this letter a shape grade (judged by eye, not by the scale)? */
function isShapeGrade(g){return !!((typeof GRADE_META==='object'&&GRADE_META[g]||{}).shape);}
/** The face a grade shows on a button or a chip — '🍌' for banana, the letter otherwise. */
function gShort(g){return ((typeof GRADE_META==='object'&&GRADE_META[g])||{}).short||g;}
/** The full name — 'Banana' / 'Grade A'. Use in dropdowns, tables and reports. */
function gLabel(g){return ((typeof GRADE_META==='object'&&GRADE_META[g])||{}).label||('Grade '+g);}
...
function dispTotals(retId,src){
  ...
  (src||dispLines()).forEach(l=>{
    ...
    lines.push({clone:l.clone,clone_name:CLONE_NAME[l.clone]||l.clone,grade:l.grade,
      ...
      /* v3.26.0 FIX (audit D-13) — the app already works out which grade the average fruit
         weight FALLS IN, and used to throw that answer away at this exact line. It was shown
         as a bare ⚠ on the worker's own phone and went no further: not onto the event, not
         onto the Marketer's card, not into scaleBlockers(). So a basket keyed Grade A whose
         fruit average 1.30 kg — Grade B by the farm's own band table — was approved and
         invoiced at RM 40/kg instead of RM 30. On 45.5 kg that is RM 455 over-invoiced, on
         one basket, with the detection already made and discarded. It now travels. */
      band_grade:c.sugg||'', band_ok:(!c.sugg||c.sugg===l.grade)});});
...
function retailerListHtml(){
  const own=canSetCredit(), act=activeRetailers(), susp=suspendedRetailers();
...
function retailerCardHtml(id){
  const r=retailerById(id), c=retailerLedger(id), own=canSetCredit();
...
function settingValue(k){
  ...
  if(k==='progover')  return PROG_OVER;
...
async function applySetting(k,v){
  try{
    ...
    if(k==='baskets'){
      /* v3.29.7 - THIS USED TO SILENTLY DELETE ANY BASKET THE FARM ADDED. It mapped over
         BASKET_SEED and nothing else, so a basket created on one device survived exactly
         until the next sync, when the incoming list was filtered back down to the three
         shipped ids and the new one vanished - on every device, with no message. Now the
         seed is OVERLAID (so a basket added in a later release still appears, and a seed
         basket cannot be lost by an old device sending a short list) and every EXTRA
         basket in the incoming list is carried through. Order: seed first, then additions
         in the order they were made, so the scale card does not reshuffle under a thumb. */
      if(!Array.isArray(v)||!v.length)return false;
      const seen={};
      const merged=BASKET_SEED.map(seed=>{
        seen[String(seed.id)]=1;
        const got=v.find(x=>x&&String(x.id)===String(seed.id));
        return got?{...seed,tare_kg:+got.tare_kg||0,name:got.name||seed.name}:{...seed};});
      v.forEach(x=>{
        if(!x||!x.id)return;
        const id=String(x.id); if(seen[id])return;
        seen[id]=1;
        merged.push({id:id, name:String(x.name||id), tare_kg:+x.tare_kg||0, ic:x.ic||'🧺'});});
      BASKETS=merged;
    ...
    if(k==='aialloc'){
      ...
      return true;}
    if(k==='progover'){
      // same merge rule as agrodrafts: per record, newest `at` wins. A REMOVED set is a
      // tombstone, not an absence — it must travel, or a phone that never saw the removal
      // would keep sending the crew to a job the Owner cancelled.
      if(!v||typeof v!=='object')return false;
      Object.keys(v).forEach(id=>{
        const inc=v[id]; if(!inc)return;
        const mine=PROG_OVER[id];
        if(!mine||String(inc.at||'')>String(mine.at||''))PROG_OVER[id]=inc;});
      if(db)await put('kv',{k:'progover',v:PROG_OVER});
      applyProgOverrides();
      if(typeof renderTimeline==='function')renderTimeline();
      if(typeof renderTaskNotice==='function')renderTaskNotice();
      return true;}
    if(k==='invover'){
      /* v3.28.0 — per PRODUCT, newest `at` wins. A retire is a tombstone and must travel:
         a phone that never saw it would keep offering the product in every picker. */
      if(!v||typeof v!=='object')return false;
      Object.keys(v).forEach(pid=>{
        const inc=v[pid]; if(!inc||typeof inc!=='object')return;
        const mine=INV_OVERRIDE[pid];
        if(!mine||String(inc.at||'')>String(mine.at||''))INV_OVERRIDE[pid]=inc;});
      if(db)await put('kv',{k:'invover',v:INV_OVERRIDE});
      applyInvOverrides();
      if(typeof refreshInventoryViews==='function')refreshInventoryViews();
...
/* v3.26.0 (audit D-13) — the grade doubt, finally shown to the person who prices the load.
   The band is recomputed here from avg fruit weight rather than trusted from the row, so it
   also works on loads weighed BEFORE this release (their lines carry no band_grade) and on
   any row that came down from the Sheet. The wording is lifted from the Marketing
   direct-dispatch screen, which has said this properly since v3.6 — it simply never
   travelled to the approval path. It WARNS, it does not block: a wrong fruit COUNT produces
   the same signal as a wrong grade, and stranding a lorry over an arithmetic guess is worse
   than making the Marketer look. */
function gradeDoubt(e){
  return reqLines(e).map(x=>{
    const avg=+x.avg_fruit_kg||((+x.fruits>0&&+x.net_kg>0)?+((+x.net_kg/+x.fruits).toFixed(2)):0);
    if(!(avg>0))return null;
    const band=x.band_grade||gradeForWeight(x.clone,avg);
    if(!band||band===x.grade)return null;
    // clone_name is set by dispTotals, but a row rebuilt from a short lines_json may not
    // carry it — fall back to the name table before the bare code, so the card never says
    // "MK" to someone who reads in "Musang King"
    return {clone:x.clone,name:x.clone_name||CLONE_NAME[x.clone]||x.clone,
            keyed:x.grade,band:band,avg:avg,
            net:+x.net_kg||0,
            gap:+(((+x.net_kg||0)*(priceOf(x.clone,x.grade,e.retailer_id)
                                  -priceOf(x.clone,band,e.retailer_id))).toFixed(2))};
  }).filter(Boolean);}
function gradeDoubtHtml(e){
  const d=gradeDoubt(e); if(!d.length)return '';
  return '<div class="gdoubt">'+
    '<div class="gdh">⚠ CHECK THE SORT BEFORE YOU APPROVE</div>'+
    d.map(x=>'<div class="gdr"><b>'+esc(x.name)+'</b> — keyed <b>Grade '+esc(x.keyed)+'</b>, '+
      'but '+nf(x.net)+' kg over '+Math.round(x.net/x.avg)+' fruit averages <b>'+nf(x.avg)+' kg</b>, '+
      'which falls in <b>Grade '+esc(x.band)+'</b>.'+
      (x.gap>0?(' Approving as keyed invoices <b>'+rm(x.gap)+'</b> more than Grade '+esc(x.band)+'.'):'')+
      '</div>').join('')+
    '<div class="gdf">Either the sort is wrong or the fruit count is. Return it to the worker '+
      'if you are not sure.</div>'+
  '</div>';}
    ...
    (!open?'':(
      ...
      '<button class="bigbtn" '+((!seen||unpriced.length||(short&&!OVR_OK))?'disabled ':'')+
        ...
        (!seen?'🔒 AUDIT THE PHOTO FIRST'
              ...
              /* v3.25.1 — this label used to restate the whole problem directly beneath a
...
/* v3.29.3 — the promise above ("stay visible for a day") was never in the code. It took the
   newest ten decided loads and kept them for ever, so a 4 August trial weighing - a photo of
   a plate of food - was still sitting on the Gate phone on 9 August, stacked above the real
   ones. The window is real now, and it is measured from the DECISION rather than the
   weighing: a load weighed last week and decided this morning is precisely the one somebody
   needs to look at. Whatever the window hides is counted out loud underneath. A list that
   silently stops short is how people come to trust a partial view of their own farm. */
const DECIDED_DAYS=2;
function decWhen(x){
  return normDt((x.d.ev&&x.d.ev.dt)||(x.d.remote&&x.d.remote.dt)||x.e.dt||'');}
function verifyHistory(){
  const all=dispatchRequests().map(e=>({e:e,d:reqDecision(e.uuid)}))
    .filter(x=>x.d.state!=='PENDING')
    .sort((a,b)=>decWhen(b).localeCompare(decWhen(a)));
  if(!all.length)return '';
  const cut=ymd(addDays(new Date(),-DECIDED_DAYS));
  const rows=all.filter(x=>decWhen(x).slice(0,10)>=cut).slice(0,10);
  const hid=all.length-rows.length;
  return '<div class="sec" style="margin-top:15px">Recently decided</div>'+
    (rows.length?'':'<div class="exphint">Nothing decided in the last '+DECIDED_DAYS+' days.</div>')+
    ...
    (hid?('<div class="exphint" style="margin-top:6px">'+hid+' older decided load'+
      (hid>1?'s are':' is')+' not shown here. Every one of them is still in the Google Sheet.</div>'):'');}
...
async function mergeDispatchDecisions(list){
  ...
  for(const x of list){
    ...
    const cur=REQ_DECIDED[u], nd=normDt(x.dt);
    // v3.29.4 — the dt test is new. Without it a row stored in the old Date.toString() form
    // would never be rewritten, because the state had not changed, and the bad text would
    // sit on the phone for ever.
    if(cur&&cur.state===st&&cur.dt===nd)continue;    // already known and stored correctly
    REQ_DECIDED[u]={state:st,
      dt:nd, by:String(x.by||''), reason:String(x.reason||''),
...
function renderMktLedger(){
  ...
  box.innerHTML=
    ...
    'the Owner or the Gate posts a credit top-up with the reason, so the mistake and the fix both stay on the record.</p>';}
...
function openRetForm(id){
  if(!canSetCredit()){toast('Only the Owner or the Gate can add or edit a retailer',1);return;}
...
async function topUpRetailer(id,fromForm){
  if(!canSetCredit()){toast('Only the Owner or the Gate can add credit',1);return;}
  const r=retailerById(id); if(!r)return;
...
function auditQueue(){return EVENTS.filter(e=>
  (e.type==='LOG_VOID'||e.type==='YIELD_ACK'||e.type==='ADMIN_PURGE'||e.type==='ADMIN_CLEANUP'
   ...
   ||e.type==='DISPATCH_CANCEL'
   // v3.27.1 — a price change is an audit row, not a farm event. Riding this key means the
   // AUDIT_LOG tab picks it up with no backend change: every field it uses (targetUuid,
   // targetType, targetWorker, detail, reason, worker, workerId, device) is already in AUD_HEAD.
   ||e.type==='PRICE_EDIT'
   // v3.28.0 — a retire and a brand swap are audit rows, not farm events. Same reasoning as
   // PRICE_EDIT: every field they use is already in AUD_HEAD, so no backend change.
   ||e.type==='PRODUCT_RETIRE'||e.type==='BRAND_SWAP')&&!e.synced);}
...
/* ================= v3.29.0 · BULK BACKDATE ==========================================
   Two passes on purpose. mdbBulkCheck() parses and validates EVERYTHING and writes nothing;
   mdbBulkSave() only runs on the checked result. A half-written import is far worse than a
   refused one — you cannot tell by looking which half landed. */
let BULK=null;
function mdbBulkParse(txt){
  const rows=[],bad=[];
  String(txt||'').split(/\r?\n/).forEach((raw,i)=>{
    const line=raw.trim(); if(!line||line.startsWith('#'))return;
    const p=line.split(/[\s,;\t]+/).filter(Boolean);
    const say=m=>bad.push({line:i+1,text:line,why:m});
    if(p.length<3)return say('needs three parts: tree, date, fruit');
    // tree — accept B2, B-2, b-002 and normalise to the census id
    const tm=String(p[0]).toUpperCase().match(/^([ABC])[-\s]?0*(\d{1,3})$/);
    if(!tm)return say('"'+p[0]+'" is not a tree like B-002');
    const id=tm[1]+'-'+String(+tm[2]).padStart(3,'0');
    if(!treeById(id))return say(id+' is not in the census');
    // date — 5/8 · 5/8/26 · 5-8 · 2026-08-05
    let dt=null,d=String(p[1]);
    let m=d.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
    if(m)dt=m[1]+'-'+String(+m[2]).padStart(2,'0')+'-'+String(+m[3]).padStart(2,'0');
    else{ m=d.match(/^(\d{1,2})[\/\-.](\d{1,2})(?:[\/\-.](\d{2,4}))?$/);
      if(m){const y=m[3]?(+m[3]<100?2000+ +m[3]:+m[3]):(new Date().getFullYear());
        dt=y+'-'+String(+m[2]).padStart(2,'0')+'-'+String(+m[1]).padStart(2,'0');}}
    if(!dt)return say('"'+p[1]+'" is not a date like 5/8');
    if(!/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/.test(dt))return say(dt+' is not a real date');
    if(dt>todayStr())return say(dt+' is in the future');
    const n=Number(p[2]);
    if(!isFinite(n)||n<0||n!==Math.floor(n))return say('"'+p[2]+'" is not a whole number of fruit');
    if(n===0)return;                     // a date with no number = nobody tied. Nothing to log.
    rows.push({tree:id,dt:dt,n:n});});
  return {rows:rows,bad:bad};}

function mdbBulkCheck(){
  const err=$('bk-bulk-err'), prev=$('bk-bulk-prev'), go=$('bk-bulk-go');
  err.textContent=''; prev.innerHTML=''; go.disabled=true; BULK=null;
  if(!canMasterAdmin()){err.textContent='Owner only';return;}
  const {rows,bad}=mdbBulkParse($('bk-bulk').value);
  if(bad.length){
    err.innerHTML='<b>'+bad.length+' line'+(bad.length>1?'s':'')+' cannot be read. Nothing was written.</b><br>'+
      bad.slice(0,12).map(b=>'line '+b.line+': '+esc(b.why)+' — <code>'+esc(b.text)+'</code>').join('<br>')+
      (bad.length>12?('<br>…and '+(bad.length-12)+' more'):'');
    return;}
  if(!rows.length){err.textContent='Nothing to log.';return;}
  /* Already-in check. TIE rows travel UP only, so this phone can only see what IT holds —
     say so plainly rather than implying the check covers every device. */
  const have={};
  EVENTS.forEach(e=>{if(e.type==='TIE')have[e.tree+'|'+String(e.dt).slice(0,10)]=(have[e.tree+'|'+String(e.dt).slice(0,10)]||0)+(+e.n||0);});
  const dupes=rows.filter(r=>have[r.tree+'|'+r.dt]!==undefined);
  const days={},trees={};
  rows.forEach(r=>{days[r.dt]=(days[r.dt]||0)+r.n;trees[r.tree]=1;});
  const dd=Object.keys(days).sort();
  BULK={rows:rows,note:String($('bk-bulk-note').value||'').trim()};
  go.disabled=false;
  prev.innerHTML='<div class="cnote" style="margin-top:9px"><b>Ready:</b> '+rows.length+
      ' round'+(rows.length===1?'':'s')+' · <b>'+nf(rows.reduce((s,r)=>s+r.n,0))+'</b> fruit · '+
      Object.keys(trees).length+' trees · '+dd[0]+' to '+dd[dd.length-1]+'</div>'+
    (dupes.length?('<div class="pinerr" style="margin-top:7px">⚠ '+dupes.length+
      ' of these already exist ON THIS PHONE for the same tree and day: '+
      dupes.slice(0,6).map(d=>esc(d.tree)+' '+d.dt).join(', ')+(dupes.length>6?' …':'')+
      '<br>Logging them again would double the count. This phone cannot see what other '+
      'devices hold, so it is not a complete check.</div>'):'')+
    '<div class="tblwrap" style="margin-top:7px"><table class="tbl"><tr><th>Day</th><th class="num">Trees</th><th class="num">Fruit</th></tr>'+
      dd.map(d=>'<tr><td>'+esc(d)+'</td><td class="num">'+rows.filter(r=>r.dt===d).length+
        '</td><td class="num">'+nf(days[d])+'</td></tr>').join('')+'</table></div>';}

async function mdbBulkSave(){
  if(!canMasterAdmin()){toast('Owner only',1);return;}
  if(!BULK||!BULK.rows.length){toast('Press CHECK first',1);return;}
  const rows=BULK.rows, note=BULK.note;
  if(!confirm('Log '+rows.length+' backdated tying rounds ('+nf(rows.reduce((s,r)=>s+r.n,0))+
    ' fruit)?\n\nEvery row is marked BACKDATED and carries the day the work was done plus '+
    'this moment as when it was keyed.\n\nThey stay on this phone until you press SYNC.'))return;
  const go=$('bk-bulk-go'); go.disabled=true; go.textContent='Writing…';
  const stamp=now(); let done=0;
  try{
    for(const r of rows){
      const t=treeById(r.tree); if(!t)continue;
      await persistEvent({uuid:uuid(),type:'TIE',dt:r.dt+' 08:00',tree:t.id,lot:t.lot,
        clone:t.clone||'', n:r.n, ropeM:ropeNeeded(r.n), roundId:uuid(),
        backdated:true, enteredAt:stamp, note:note||'bulk backdate',
        worker:CFG.worker, workerId:CFG.uid||'', device:CFG.device, synced:false});
      done++;}
  }catch(ex){
    $('bk-bulk-err').textContent='Stopped after '+done+' of '+rows.length+
      ' rows. Those '+done+' ARE saved — remove them from the paste before trying again.';
    go.textContent='🕓 LOG THEM ALL'; rebuildLedgers(); badge(); refreshEverything(); return;}
  BULK=null; $('bk-bulk').value=''; $('bk-bulk-note').value='';
  $('bk-bulk-prev').innerHTML=''; go.textContent='🕓 LOG THEM ALL';
  rebuildLedgers(); badge(); refreshEverything();
  toast('✓ '+done+' rounds backdated — press SYNC to send them up');}
...
const CLN_TYPES=[
  ...
  /* v3.30.2 - rations & gifts. Without this chip the rows were still deletable (an unfiltered
     list shows every type) but arrived with a BLANK label, which is worse than not offering
     them: a row you cannot read is a row you delete by accident. */
  {k:'FOC_REQ',    t:'Rations & gifts'},
...
/* v3.30.2 - the chip says "Rations & gifts" and means the whole conversation: the request AND
   the Gate's answer. Leaving FOC_APPROVE behind would leave an approval pointing at a request
   that no longer exists - the same orphan the rope/correction expansion below exists to
   prevent. */
const CLN_FOC=['FOC_REQ','FOC_APPROVE','FOC_REFUSE'];
function clnTypeMatch(type){
  if(!CLN.types.length)return true;
  if(CLN.types.indexOf('ADJ')>=0&&CLN_ADJ.indexOf(type)>=0)return true;
  if(CLN.types.indexOf('FOC_REQ')>=0&&CLN_FOC.indexOf(type)>=0)return true;
    ...
    for(let i=0;i<pl.active.length;i++){
      ...
      for(const l of draftLines(d)){
        ...
        await persistEvent({uuid:uuid(),type:'STOCK_OUT',dt:stamp,
          ...
          natkey:natKeyRun(d.uuid,l.slot,x.lot,q,x.trees),
...
function focGateInKg(from,to){
  return +EVENTS.filter(e=>e&&e.type==='DISPATCH_REQ'&&inWindow(e.dt,from,to))
    .reduce((s,e)=>s+(+e.total_kg||0),0).toFixed(2);}
...
function focSoldKg(from,to){
  ...
  return {kg:+rows.reduce((s,e)=>s+(+e.total_kg||0),0).toFixed(2),
          rm:+rows.reduce((s,e)=>s+(+e.amount||e.total_rm||0),0).toFixed(2)};}
...
function renderFocQueue(){
  ...
  h+='<div class="sec" style="margin-top:12px">🎁 '+esc(tr('foc_give','Record fruit going out free'))+'</div>'+
     ...
     '<button class="bigbtn" onclick="focSubmit()">'+
       esc(may?tr('foc_record','RECORD IT'):tr('foc_ask','ASK THE GATE'))+'</button>';
  ...
  h+='<div class="sec" style="margin-top:14px">🧾 '+esc(tr('foc_balance','Where the fruit went — this month'))+'</div>'+
     ...
     '<tr><td>→ '+esc(tr('foc_shed','Still in the shed'))+'</td><td>'+nf(bal.shed_kg)+' kg</td><td></td></tr>'+
