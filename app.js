...
function countsLocally(e){
  // v3.27.0 — A PULLED ROW IS NEVER ADDED ON TOP. It came DOWN out of the same Sheet that
  // built `treestats`, so it is already inside the farm-wide total by definition. Counting
  // it again here would double every fruit the catch-up brought in, and — because the tree
  // ledger drives tying, securing and the dispatch allocation — the error would spread
  // straight into the loads. This one line is what makes the down-leg safe: the pull fixes
  // the screens that read the raw event list (the dashboard, the store, the unsecured-drop
  // alert, the job history) and leaves every per-tree balance exactly where it was.
...
/* ==================================================================================
   v3.27.0 · THE CATCH-UP — the missing DOWN leg
   ----------------------------------------------------------------------------------
   Every record this app has ever written travels ONE WAY. refreshMasters() brings down
   twelve master tables — trees, products, retailers, staff, programmes, tasks, the
   per-tree totals — and not one single event. A drop, a rotten fruit, a tying round, a
   stock movement, a finished job: all of it goes UP to the Sheet and never comes back.
   So two phones that each did real work held two different, permanently private ledgers,
   both looking authoritative, neither complete. That is not a bug in one screen; it is
   why the Owner's laptop and the Owner's phone have never once agreed, and it is how one
   broadcast of fertiliser came to be booked twice — once from each handset, with neither
   able to see the other's entry.

   THE THREE RULES THIS OBEYS, and none of them may be relaxed without re-testing:

   1. IT ONLY EVER ADDS. There is no delete path and no overwrite path. A uuid already on
      this phone is skipped untouched — the local copy always wins, so a row still queued
      for upload, or edited locally, can never be clobbered by an older copy from the Sheet.
      Press the button twice and the second press brings down nothing.

   2. NOTHING PULLED COUNTS TOWARDS A TREE. Pulled rows carry `pulled:true`, and
      countsLocally() returns false for them, so every per-tree balance keeps coming from
      the Sheet's own `treestats` total exactly as before. What the pull fixes is every
      screen that reads the raw event list and therefore only ever saw this one phone:
      the dashboard counts, the store balance, the unsecured-drop alert, the job history.

   3. NO MONEY MOVES. MKT_DISPATCH and SALES are not in the backend's DOWN_TABS at all, so
      no invoice, price or merchant credit can arrive on any phone in any role.

   And one hard-won constraint, spelled out in database.js at STOCK_OPEN_AT: a product's
   `stock` is a SNAPSHOT taken on 5 Aug 2026, not a day-one figure. A store movement dated
   on or before it is already inside that number, so pulling it would deduct the same
   fertiliser twice. Those rows are skipped, and the skip is reported rather than hidden.
   ================================================================================== */

const dnum=v=>{const n=parseFloat(String(v==null?'':v).replace(/,/g,''));return isFinite(n)?n:0;};
const dstr=v=>(v==null?'':String(v)).trim();

/* Three of these tabs are the ORIGINAL workbook's, and their headings are the workbook's
   names, not the app's field names — EventUUID not uuid, QtyUsed not qty, TargetLot not
   lot. ensureExtraCols_ in the Apps Script places every extra column BY HEADER NAME, so
   the header row is the only honest description of a tab's shape and these three need
   translating on the way back in. Every other tab is written by upsertByUuid_ from a
   *_HEAD array of the app's own field names, so its row already IS an event and only
   needs its numbers coercing back from text. */
const DOWN_MAP={
  DROP_LOGS:r=>({type:'DROP',dt:dstr(r.DateTime),tree:dstr(r.TreeID),lot:dstr(r.Lot),
    clone:dstr(r.Clone),qty:dnum(r.Qty),grade:dstr(r.Grade),estkg:dnum(r.EstKg),
    worker:dstr(r.Worker),device:dstr(r.Device),notes:dstr(r.Notes),
    // v2.9: a drop filed before the flag existed has no Secured cell and WAS secured.
    // Only the explicit word NO makes it unsecured — an empty cell must not silently
    // turn every pre-v2.9 drop into a variance on the Owner's alert screen.
    secured:dstr(r.Secured).toUpperCase()!=='NO',
    dropKind:dstr(r.DropKind)||(dstr(r.Secured).toUpperCase()==='NO'?'UNSECURED':'SECURED'),
    pickId:dstr(r.PickId)}),
  STOCK_IN:r=>({type:'STOCK_IN',dt:dstr(r.Date),pid:dnum(r.ProductID),pname:dstr(r.ProductName),
    qty:dnum(r.QtyIn),unit:dstr(r.Unit),containers:dnum(r.Containers),cost:dnum(r.TotalRM),
    unitPrice:dnum(r.UnitPrice),supplier:dstr(r.Supplier),worker:dstr(r.ReceivedBy),
    ref:dstr(r.InvoiceRef),ai:dstr(r.ActiveIngredient),notes:dstr(r.Notes),lot:''}),
  STOCK_OUT:r=>({type:'STOCK_OUT',dt:dstr(r.DateTime),pid:dnum(r.ProductID),pname:dstr(r.ProductName),
    qty:dnum(r.QtyUsed),unit:dstr(r.Unit),set:dstr(r.SpraySet),cost:dnum(r.CostRM),
    worker:dstr(r.Worker),device:dstr(r.Device),notes:dstr(r.Notes),lot:dstr(r.TargetLot),
    ai:dstr(r.ActiveIngredient),progSet:dstr(r.Programme),tanks:dstr(r.TanksUsed),
    water:dnum(r.WaterLitres),crew:dnum(r.Crew),hours:dnum(r.LabourHours)})
};
/* Field-for-field tabs: the numeric columns, so text out of a spreadsheet cell becomes a
   number again. Anything not named here is copied across as the string it already is. */
const DOWN_NUM={
  ROTTEN_LOGS:['qty','estkg'],
  TYING_LOGS:['n','ropeM'],
  LOG_ADJUST:['was','now','delta','estkg'],
  TIE_ADJUST:['was','now','delta','ropeM'],
  STOCK_ADJUST:['pid','systemQty','counted','variance','delta','varianceValueRM','cost','qty'],
  TASK_LOGS:['count','trees','crew','hours','manHours']
};
function downRowToEvent(r){
  const tab=dstr(r._tab), id=dstr(r._id);
  if(!tab||!id)return null;
  let ev;
  if(DOWN_MAP[tab]){ ev=DOWN_MAP[tab](r); }
  else{
    ev={};
    for(const k in r){ if(k.charAt(0)==='_')continue; ev[k]=r[k]; }
    (DOWN_NUM[tab]||[]).forEach(k=>{ if(ev[k]!=null&&ev[k]!=='')ev[k]=dnum(ev[k]); });
    // STOCK_ADJUST is written with `systemQty`/`variance` headings but onHand() reads
    // `before`/`delta`. Without this the adjustment would land with a delta of zero and
    // a stock-take done on another phone would arrive looking like it changed nothing.
    if(tab==='STOCK_ADJUST'){
      if(ev.before==null&&ev.systemQty!=null)ev.before=dnum(ev.systemQty);
      if(ev.delta==null&&ev.variance!=null)ev.delta=dnum(ev.variance);
      if(ev.cost==null&&ev.varianceValueRM!=null)ev.cost=dnum(ev.varianceValueRM);
    }
    ev.type=dstr(r._type)||dstr(ev.type);
  }
  if(!ev||!ev.type)return null;
  ev.uuid=id;
  if(!ev.dt)ev.dt=dstr(r._dt);
  // Down-leg rows are, by definition, already in the Sheet. Marking them synced keeps them
  // out of doSync's upload batch — a pulled row must never be pushed back up.
  ev.synced=true; ev.syncedAt=''; ev.pulled=true; ev.pulledAt=now();
  return ev;
}
const STOCK_TYPES=['STOCK_IN','STOCK_OUT','STOCK_ADJUST'];
let pulling=false;
function cuSay(html){const el=$('catchupsay');if(el){el.style.display=html?'block':'none';el.innerHTML=html||'';}}
async function pullEvents(manual){
  if(pulling)return;
  if(!CFG||!CFG.url){if(manual)toast(tr('t27_cu_nourl'),1);return;}
  if(!navigator.onLine){if(manual)toast(tr('t27_cu_offline'),1);return;}
  pulling=true;
  const b=$('catchupbtn'); const label=b?b.textContent:'';
  if(b){b.disabled=true;b.textContent=tr('t27_cu_run');}
  cuSay('');
  try{
    const q='?events=1&cap=3000&role='+encodeURIComponent(myRole())+
            '&uid='+encodeURIComponent((CFG&&CFG.uid)||'');
    const r=await fetchT(CFG.url+q,{},SYNC_TIMEOUT_MS);
    const j=await r.json();
    // An Apps Script deployment that predates v3.27.0 knows nothing about ?events=1 and
    // answers with the ordinary masters payload — which has ok:true and no `events` flag.
    // Without this check that reply would read as "nothing new" and the Owner would press
    // the button all week believing the two phones were level.
    if(!j||!j.ok||!j.events||!Array.isArray(j.rows))throw new Error(tr('t27_cu_old'));
    const have=new Set(EVENTS.map(e=>String(e.uuid)));
    const got={}, add=[]; let dup=0, oldStock=0, bad=0;
    for(const row of j.rows){
      const ev=downRowToEvent(row);
      if(!ev){bad++;continue;}
      if(have.has(String(ev.uuid))){dup++;continue;}
      // the opening-balance guard — see STOCK_OPEN_AT in database.js
      if(STOCK_TYPES.indexOf(ev.type)>=0&&String(ev.dt||'').slice(0,10)<=STOCK_OPEN_AT){oldStock++;continue;}
      have.add(String(ev.uuid));            // guards against a duplicate uuid inside one reply
      add.push(ev);
      got[ev.type]=(got[ev.type]||0)+1;
    }
    for(const ev of add){ EVENTS.push(ev); if(db)await put('events',ev); }
    if(!db)mem.events=EVENTS;
    rebuildLedgers(); badge(); renderSync();
    if(typeof refreshInventoryViews==='function')refreshInventoryViews();
    if(typeof renderDash==='function'&&$('k-drops'))renderDash();
    const lines=[];
    if(add.length){
      lines.push('<b>'+tr('t27_cu_from')+' '+add.length+'</b>');
      lines.push(Object.keys(got).sort().map(k=>'· '+downLabel(k)+' — <b>'+got[k]+'</b>').join('<br>'));
    } else lines.push('<b>'+tr('t27_cu_none2')+'</b>');
    const notes=[];
    if(dup)notes.push(dup+' '+tr('t27_cu_skipdup'));
    if(oldStock)notes.push(oldStock+' '+tr('t27_cu_skipstock'));
    if(j.capped)notes.push(tr('t27_cu_capped'));
    if(notes.length)lines.push('<span class="small" style="opacity:.75">'+notes.join('<br>')+'</span>');
    cuSay(lines.join('<br>'));
    if(manual)toast(add.length?('✓ '+add.length+' '+tr('t27_cu_got')):tr('t27_cu_none'),0);
    return add.length;
  }catch(err){
    cuSay('<b style="color:#b3261e">'+esc(err.message||tr('t27_cu_fail'))+'</b>');
    if(manual)toast(err.message||tr('t27_cu_fail'),1);
    return 0;
  }finally{
    pulling=false;
    if(b){b.disabled=false;b.textContent=label||tr('t27_cu_btn');}
  }
}
function downLabel(t){
  return ({DROP:'Fruit drops',ROTTEN:'Rotten fruit',TIE:'Tying rounds',
    DROP_ADJUST:'Drop corrections',ROTTEN_ADJUST:'Rotten corrections',
    TIE_ADJUST:'Tying corrections',STOCK_IN:'Stock received',
    STOCK_OUT:'Stock used',STOCK_ADJUST:'Stock-take',TASK_DONE:'Jobs finished'})[t]||t;
}
