...
/* ===================== v3.30.0 - THE FOUR DEVICES, BY JOB NOT BY PERSON =====================
   Owner's decision, 7 Aug 2026: one device per JOB, not one per person. The v3.27.0
   duplicate guard compares the device name written on a row against the device that filed
   that job first; two machines both carrying the factory default 'phone-01' read as ONE
   machine and the duplicate is waved straight through - the 85 kg MSolumax double-count
   with the safety catch disengaged. So the name is now a CLOSED LIST. A device whose name
   is not here cannot upload (it can still record - see deviceNamed() in app.js).
   Matching ignores capitals; the spelling stored and sent is always the one written here,
   so FIELD-A07 and field-a07 can never become two devices on the Sheet.
   To add a fifth device, add it HERE and nowhere else.
   ========================================================================================= */
const DEVICE_NAMES=['owner-laptop','Gate-Iphone','field-a07','purchase-PC'];

/* ===================== v3.30.0 - THE DATE FLOOR FOR THE RETURN ROAD =====================
   ...
   travel. The floor exists because the 7 Aug 2026 data audit found real rows and pre-trial
   test rows mixed together in the same tabs: without it, every device would pull the whole
   lot down on the first sync of the trial and there would be no way to tell them apart
   afterwards. Move this forward to start a clean period; never move it backward without
   deciding first what happens to the rows in between.
   ===================================================================================== */
